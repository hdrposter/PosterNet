...
1. Learning a Deep Single Image Contrast Enhancer from Multi-Exposure Images
   ...
   该论文研究的是针对单张图像进行优化的卷积神经网络。数据集是照片对，由一张暗曝光的照片和一张HDR照片组成的，暗曝光作为输入，HDR照片作为Ground Truth，从而由此训练出合适的卷积神经网络。在输入网络时，该论文首先将照片从按频域分割成低频和高频两部分，再分别训练出2个神经网络（对应低频和高频照片的输出），再将该2个神经网络的输出合并后继续训练出一条神经网络生成优化后的照片。
...
根据老师的建议，我们准备从简单的融合下手，而我们想试试最后用到的OpenCV的融合算法，于是拍摄了几组不同曝光的照片，发现效果并不好。光照没有很大改善，ppt有些**灰蒙蒙的**，且容易产生鬼影。因为不可能要求演讲人不动。

（效果图）

然后我们想到之前应用到的林会东看到那片论文的网络出来的效果除了ppt区域有些过曝以外，其他部分的光照还算可以，**而且可以解决鬼影问题**，我们如果可以针对ppt区域，将融合好的图片和看的清ppt区域的中等曝光区域做融合就好了。
